...
contract MyEscrow is AccessControl, ReentrancyGuard {
    ...
    enum EscrowBool { TIMELOCKED, UNLOCKED, IN_DISPUTE, TERMINATED }
    ...
    event EscrowProposalInDispute(uint256 id, EscrowBool inDispute, uint256 deadline);
    ...
    function depositToEscrow(uint256 _escrowId) 
        public payable _isSender(_escrowId)  _isNotPerma(){
    ...
    function _isEscrowSender(uint256 _eId) internal view returns (bool) {
        bool isOk = false;
        for (uint i = 0; i < escrowList[_eId].senders.length;i++) {
    ...
    /// @param _eId represents the escrow id.
    function _isEscrowReceiver(address _receiv, uint256 _eId) internal view returns (bool){
        ...
        returns (bool)
    {
        ...
        uint totalBalance = helper.balance;
    ...
    public payable _isSender(_eId) _isNotPerma() returns (uint256) {
        ...
        //event
        emit EscrowProposalInDispute(_eId, EscrowBool.IN_DISPUTE, proposal.deadline);
        //if all ok, return
        return numProposals;
    }
    ...
    function hasVoted(uint256 proposalIndex) public view returns (bool){
        return proposals[proposalIndex].voters[msg.sender] == true  ||
        ...
        _isNotPerma()
    {
        ...
        require(numVotes < escrowList[proposalIndex].senders.length, 
    ...
    emit EscrowTerminated(_proposalIndex, EscrowBool.TERMINATED, proposals[_proposalIndex].deadline);
    }
        ...
        returns (bool)
    {
        ...
        require(proposals[_eId].executed == true &&
        ...
        require(
        _isExpired(_eId) && 
        helper.locked == EscrowBool.UNLOCKED
        ,  
        "The escrow doen't have the conditions to be withdrawed.");
    ...
    /// @param _eId represent the escrowId pointer.
    function twoPerCentOfBalanceEscrow(uint256 _eId) internal view returns (uint256){
    ...
    function setEscrowBool(uint256 _eId, EscrowBool _newBool) public onlyRole (ROLE_PAUSER) {
